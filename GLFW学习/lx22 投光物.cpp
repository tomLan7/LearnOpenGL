...
int main()
{
    ...
    ShaderProgram* shaderProgram = ShaderProgram::CreateFromVertexAndFragmentPath("lx21.vert", "lx22.frag");
    ...
    while (!window->IsShouldClose())
    {
        ...
        shaderProgram->Uniform("light.ambient", vec3(0.4f, 0.4f, 0.4f));
        shaderProgram->Uniform("light.diffuse", vec3(0.65f, 0.65f, 0.65f));
        shaderProgram->Uniform("light.specular", vec3(1.0f, 1.0f, 1.0f));
        shaderProgram->Uniform("light.position", TransMat * vec4(lightPos, 1));
        shaderProgram->Uniform("light.constant",1.0f);
        shaderProgram->Uniform("light.linear", 0.14f);
        shaderProgram->Uniform("light.quadratic", 0.07f);
