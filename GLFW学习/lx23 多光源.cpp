...
int main()
{
    ...
    ShaderProgram* shaderProgram = ShaderProgram::CreateFromVertexAndFragmentPath("lx21.vert", "lx23.frag");
    ...
    while (!window->IsShouldClose())
    {
        ...
        shaderProgram->Uniform("dirLight.ambient", vec3(0.1f, 0.1f, 0.1f));
        shaderProgram->Uniform("dirLight.diffuse", vec3(0.65f, 0.65f, 0.65f));
        shaderProgram->Uniform("dirLight.specular", vec3(1.0f, 1.0f, 1.0f));
        shaderProgram->Uniform("dirLight.direction", -lightPos);
        
            for (int i = 0; i < 4;i++) {
                char buff[50];
                sprintf(buff,"pointLights[%d]",i);
                shaderProgram->Uniform(buff+string(".position"), pointLightPositions[i]);
                shaderProgram->Uniform(buff + string(".constant"), 1.0f);
                shaderProgram->Uniform(buff + string(".linear"), 0.14f);
                shaderProgram->Uniform(buff + string(".quadratic"), 0.07f);

                shaderProgram->Uniform(buff + string(".ambient"), vec3(0.4f, 0.4f, 0.4f));
                shaderProgram->Uniform(buff + string(".diffuse"), vec3(0.65f, 0.65f, 0.65f));
                shaderProgram->Uniform(buff + string(".specular"), vec3(1.0f, 1.0f, 1.0f));
            }
        
        shaderProgram->Uniform("viewPos", camera->pos);
        shaderProgram->Uniform("spotLight.position", camera->pos);
        shaderProgram->Uniform("spotLight.direction", camera->direction());
        shaderProgram->Uniform("spotLight.cutOff", cos(glm::radians(7.f)));
        shaderProgram->Uniform("spotLight.outerCutOff", cos(glm::radians(14.f)));
        shaderProgram->Uniform("spotLight.constant",1.0f);
        shaderProgram->Uniform("spotLight.linear", 0.045f);
        shaderProgram->Uniform("spotLight.quadratic", 0.0075f);
        shaderProgram->Uniform("spotLight.ambient", vec3(0.4f, 0.4f, 0.4f));
        shaderProgram->Uniform("spotLight.diffuse", vec3(0.65f, 0.65f, 0.65f));
        shaderProgram->Uniform("spotLight.specular", vec3(1.0f,1.0f, 1.0f));
        ...
        shaderProgram->Uniform("material.ambient", 0);
        ...
        for (int i = 0; i < 4;i++) {
            lightModel = translate(mat4(1), pointLightPositions[i]);
